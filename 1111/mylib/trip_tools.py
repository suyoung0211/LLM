...
def run_agent(user_input: str, session_id: str):
    ...
    # 세션을 계속 가져오는 이유
    '''
    세션 기반 대화 기록은 InMemoryChatMessageHistory 객체 안에 저장
    사용자가 이전에 어떤 질문을 했는지, AI가 어떤 답변을 했는지를 LLM이 참고할 수 있도록 전달해야 함

    같은 session_id로 히스토리를 가져 옴
            ↓
    여기에 새 메시지를 추가
            ↓
    이전 메시지와 함께 LLM에 전달

    이렇게 해야 AI가 “이전에 어떤 질문이 있었는지”를 기억하고 답변 가능
    '''
    try:
        # 1️⃣ 세션 히스토리 가져오기
